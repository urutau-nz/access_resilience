...
file_path = r'results/recovery/new_road_h_recovery_{}_{}.csv'.format(optimise_service, optimise_demographic)
...
def recover():
    ...
    # greedy optimise
    results = init_recovery(df_options, results, demo, road_options, db, context)
    # save results
...
def init_recovery(df_options, results, demo, road_options, db, context):
    '''
    ...
    for i in tqdm(iterations): # total number of iterations
        ...
        ## if supo can reach
        if 1 in list(df_options['proximity']):
            best_id = list(df_options[(df_options['proximity']==1)&(df_options['type']=='service')]['option_id'])[0]
            best_type = 'service'
            # major arterial
        elif df_options['major'].sum()!=0:
            if (len(df_options[df_options['major']==df_options['major'].max()])==1):
                best_id = list(df_options[df_options['major']==df_options['major'].max()]['option_id'])[0]
                best_type = 'road'
            else:
                temp_df = df_options[(df_options['option_id'].isin(list(df_options[df_options['major']==df_options['major'].max()]['option_id'])))]
                best_id = list(temp_df[temp_df['minor']==temp_df['minor'].max()]['option_id'])[0]
                best_type = 'road'
        # minor
        elif (df_options['minor'].sum()!=0):
            if (len(df_options[df_options['minor']==df_options['minor'].max()])==1):
                best_id = list(df_options[df_options['minor']==df_options['minor'].max()]['option_id'])[0]
                best_type = 'road'
            else:
                temp_df = df_options[(df_options['option_id'].isin(list(df_options[df_options['minor']==df_options['minor'].max()]['option_id'])))]
                best_id = list(temp_df[temp_df['collector']==temp_df['collector'].max()]['option_id'])[0]
                best_type = 'road'
        # collector
        elif (df_options['collector'].sum()!=0):
            if (len(df_options[df_options['collector']==df_options['collector'].max()])==1):
                best_id = list(df_options[df_options['collector']==df_options['collector'].max()]['option_id'])[0]
                best_type = 'road'
            else:
                temp_df = df_options[(df_options['option_id'].isin(list(df_options[df_options['collector']==df_options['collector'].max()]['option_id'])))]
                best_id = list(temp_df[temp_df['local']==temp_df['local'].max()]['option_id'])[0]
                best_type = 'road'
        # local
        elif (df_options['local'].sum()!=0):
            best_type = 'road'
            if (len(df_options[df_options['local']==df_options['local'].max()])==1):
                best_id = list(df_options[df_options['local']==df_options['local'].max()]['option_id'])[0]
                best_type = 'road'
            else:
                temp_df = df_options[(df_options['option_id'].isin(list(df_options[df_options['local']==df_options['local'].max()]['option_id'])))]
                best_id = list(temp_df[temp_df['private']==temp_df['private'].max()]['option_id'])[0]
                best_type = 'road'
        # local
        elif (df_options['private'].sum()!=0):
            if (len(df_options[df_options['private']==df_options['private'].max()])==1):
                best_id = list(df_options[df_options['private']==df_options['private'].max()]['option_id'])[0]
                best_type = 'road'
            else:
                temp_df = df_options[(df_options['option_id'].isin(list(df_options[df_options['private']==df_options['private'].max()]['option_id'])))]
                best_id = list(temp_df[temp_df['private']==temp_df['private'].max()]['option_id'])[0]
                best_type = 'road'
        ...
        # run query for supermarket supply to 172.45050,-43.56613 or 172.66500,-43.34904
        supply_distances = query_supply_points(dest_ids, db, context, optimise_service=None)
        # take unique ids not nan and less than 25km
        proximity_ids = []
        for i in list(supply_distances['id_orig'].unique()):
            if (True in list(supply_distances[supply_distances['id_orig']==i].distance.isnull())) or (supply_distances[supply_distances['id_orig']==i].distance.sum() > 60000):
                continue
